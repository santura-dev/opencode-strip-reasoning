...
function formatStatsReport(sessionID: string): string {
  ...
  if (calls === 0) {
    try {
      ...
      return "No strip-reasoning stats yet. The plugin will track stats after the model makes LLM calls with prior reasoning in history. Try again after a few turns with a reasoning model."
    }
  }
  ...
  const lines: string[] = []
  lines.push("**strip-reasoning** \u2014 this session")
  lines.push("")
  lines.push("| metric | value |")
  lines.push("|---|---|")
  lines.push(`| LLM calls processed | ${calls} |`)
  lines.push(`| Calls with reasoning stripped | ${callsWithReasoning} |`)
  lines.push(`| Reasoning chars stripped | ${totalReasoningCharsStripped.toLocaleString()} |`)
  lines.push(`| Est. tokens saved | ${totalEstimatedTokensSaved.toLocaleString()} |`)
  lines.push(`| Peak reasoning in one call | ${peakReasoningChars.toLocaleString()} |`)
  ...
  if (stats && stats.byCall.length > 0) {
    ...
    lines.push("")
    lines.push("| call | tokens saved | context share |")
    lines.push("|---|---|---|")
    stats.byCall.slice(-5).forEach((call, i) => {
      if (call.reasoningCharsBefore > 0) {
        const pct =
          call.textChars + call.reasoningCharsBefore > 0
            ? (
                ((call.reasoningCharsBefore - call.reasoningCharsAfter) /
                  (call.reasoningCharsBefore + call.textChars)) *
                100
              ).toFixed(0) + "%"
            : "100%"
        lines.push(
          `| #${stats.byCall.length - Math.min(5, stats.byCall.length) + i + 1} | ${call.tokensSaved.toLocaleString()} | ${pct} |`,
        )
      } else {
        lines.push(
          `| #${stats.byCall.length - Math.min(5, stats.byCall.length) + i + 1} | 0 | (no reasoning) |`,
        )
      }
    })
  }
...
const plugin: Plugin = async (_input, options) => {
  ...
  if (!VALID_MODES.includes(mode)) {
    console.error(
      `opencode-strip-reasoning: invalid mode "${String(mode)}", falling back to "strip". valid modes: ${VALID_MODES.join(", ")}`,
    )
    mode = "strip"
  }
  if (!Number.isInteger(keepCount) || keepCount < 0) {
    console.error(
      `opencode-strip-reasoning: invalid keepCount "${String(keepCount)}", falling back to 0.`,
    )
    keepCount = 0
  }
  ...
  const hooks: Hooks = {
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (mode === "strip") {
        ...
        for (const msg of output.messages) {
          for (let i = 0; i < msg.parts.length; i++) {
            ...
            if (part.type === "reasoning") {
              ;(part as { type: "reasoning"; text: string }).text = "[reasoning omitted]"
            }
          }
        }
      ...
      const nonEmpty = output.messages.filter((msg) => msg.parts.length > 0)
      ...
      log({
        ...
        estimatedTokensSavedPercent:
          before.reasoningChars > 0
            ? (
                (savedChars / (before.reasoningChars + before.textChars + before.toolParts * 100)) *
                100
              ).toFixed(1) + "%"
            : "0%",
      })
    ...
    tool: {
      strip_reasoning_stats: tool({
        description:
          "Show reasoning token savings for the current session. Displays how many reasoning tokens were stripped from conversation history by the strip-reasoning plugin.",
        args: {},
